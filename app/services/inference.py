...
class ONNXInferenceEngine:
    ...
    def load(self):
        try:
            ...
            if not lstm_path.exists() or not tf_path.exists():
                logger.warning(f"ONNX models not found in {self.model_dir} [G stub mode")
                return
            self.lstm_session = ort.InferenceSession(str(lstm_path), opts)
            self.tf_session   = ort.InferenceSession(str(tf_path),   opts)
            if meta_path.exists():
                self.meta_session = ort.InferenceSession(str(meta_path), opts)
            self.loaded = True
    ...
    def predict(self, features: np.ndarray) -> Dict:
        if not self.loaded:
            return self._stub()
        try:
            x = features[-SEQ_LEN:].astype(np.float32)
            x = np.expand_dims(x, axis=0)
            lstm_out  = self.lstm_session.run(None, {self.lstm_session.get_inputs()[0].name: x})
            tf_out    = self.tf_session.run(None,   {self.tf_session.get_inputs()[0].name:   x})
            lstm_cls  = lstm_out[1]
            tf_cls    = tf_out[1]
            if self.meta_session:
                meta_in  = np.hstack([lstm_cls, tf_cls]).astype(np.float32)
                meta_out = self.meta_session.run(None, {self.meta_session.get_inputs()[0].name: meta_in})
                probs = meta_out[0][0]
            else:
                probs = (lstm_cls[0] * 0.45 + tf_cls[0] * 0.55)
            c24, c48, c72 = float(probs[0]), float(probs[1]), float(probs[2])
            ...
            return {"signal": signal, "confidence": avg, "conf_24h": c24, "conf_48h": c48, "conf_72h": c72, "loaded": True}
        except Exception as e:
            logger.error(f"Inference error: {e}")
            return self._stub()

    def _stub(self):
        return {"signal": "LOW_CONFIDENCE", "confidence": 0.0, "conf_24h": 0.0, "conf_48h": 0.0, "conf_72h": 0.0, "loaded": False}
...
def build_features_from_candles(candles: List[Dict]) -> Optional[np.ndarray]:
    if len(candles) < SEQ_LEN + 10: return None
    ...
    f[1:,0] = np.diff(o)/(o[:-1]+1e-8)
    f[1:,1] = np.diff(h)/(h[:-1]+1e-8)
    f[1:,2] = np.diff(l)/(l[:-1]+1e-8)
    f[1:,3] = np.diff(c)/(c[:-1]+1e-8)
    vm = np.mean(v)+1e-8
    f[:,4] = v/vm
    v20 = np.convolve(v, np.ones(20)/20, mode='same')
    f[:,5] = v/(v20+1e-8)
    ...
    e10,e20,e34 = _ema(c,10),_ema(c,20),_ema(c,34)
    ...
    sl = _ema(np.nan_to_num(ml),9)
    f[:,15] = np.nan_to_num(sl/(c+1e-8))
    f[:,16] = f[:,14]-f[:,15]
    tr = np.maximum(h[1:]-l[1:], np.maximum(np.abs(h[1:]-c[:-1]), np.abs(l[1:]-c[:-1])))
    atr = np.full(n, np.nan); 
    if len(tr)>=14: atr[14]=np.mean(tr[:14])
